<template>
    ...
    <div class="min-h-screen relative">
        <div class="w-40 transform fixed inset-y-0 left-0 transition ease-in-out duration-3000 transition-all shadow-2xl shadow-green-500/50 rounded-br-lg md:rounded-none z-10 md:visible bg-slate-600 text-white text-sm p-[4px] space-y-2 md:transform-none" :class="{'invisible -translate-x-full' : sidebar, 'translate-x-0' : !sidebar}">
                ...
                <Link :href="route('articles.index')" class="flex items-center w-full px-1 py-2 rounded-md hover:bg-slate-800 transition ease-out duration-200">
                    <ArticleIcon class="h-5 w-5 mr-2 text-gray-200" />
                    <span>Articles</span>
                </Link>
            
                <Link :href="route('invoices.index')" class="flex items-center w-full px-1 py-2 rounded-md hover:bg-slate-800 transition ease-out duration-200">
                    <PrintIcon class="h-5 w-5 mr-2 text-gray-200" />
                    <span>Factures</span>
                </Link>
            
                <Link :href="route('clients.index')" class="flex items-center w-full px-1 py-2 rounded-md hover:bg-slate-800 transition ease-out duration-200">
                    <GroupIcon class="h-5 w-5 mr-2 text-gray-200" />
                    <span>Clients</span>
                </Link>
            
                <Link href="/stats" class="flex items-center w-full px-1 py-2 rounded-md hover:bg-slate-800 transition ease-out duration-200">
                    <ChartIcon class="h-5 w-5 mr-2 text-gray-200" />
                    <span>Stats</span>
                </Link>
                <Link :href="route('logout')" method="post" as="button" class="w-full flex items-center px-1 py-2 rounded-md hover:bg-slate-800 transition ease-out duration-200">
