<template>
	<div class="flex justify-center max-w-3xl mx-auto px-2">
		...
		<div class="w-full">
			<h2 class="my-4 border-b text-center text-gray-600 font-bold text-2xl">Éditer la commande</h2>
		    <form @submit.prevent="submit" method="post" class="mx-auto relative max-w-sm">
		      <div v-if="messages.orders && messages.orders.editSuccess" class="mb-1 font-medium text-sm text-green-600 flex items-center justify-center">
		        <span>{{ messages.orders.editSuccess }}</span>
		      	<CheckIcon class="h-5 w-5 ml-1" />
		      </div>
		      <div v-if="messages.orders && messages.orders.editFailure" class="mb-1 font-medium text-sm text-red-600 text-center">
		        <span>{{ messages.orders.editFailure }}</span>
		      </div>
		      <div class="mx-auto">
...
export default {
	...
	setup(props) {
		...
		const deleteArticle = () => {
				isOpen.value = false;
      	axios.delete(route('articles.destroy', props.article.id))
      	.then((res) => {
