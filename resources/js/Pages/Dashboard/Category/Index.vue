		...
		<div class="px-2 mx-auto w-full">
			...
			<div class="max-w-2xl mx-auto rounded-xl bg-white border pt-4 my-7">
				<div class="flex items-center justify-center space-x-2 px-2 mb-4">
					...
					<!-- <Filters /> -->
					<Link :href="route('categories.create')" class="bg-blue-600 text-white py-1 px-2 text-sm shadow-md shadow-blue-500/50 flex items-center rounded-md hover:bg-blue-700 transition ease-in-out duration-300 focus:outline-none">
				...
				<table class="w-full table-auto whitespace-nowrap text-sm shadow-sm" >
					<thead>
						<tr>
							...
							<th class="py-3">
								<div class="flex items-center justify-center">
								<ChevronUpIcon @click="setFilter('sortByArticles','asc')" class="h-4 w-4 mr-1 text-gray-500 hover:text-amber-700 cursor-pointer" :class="{'text-amber-700': form.sortByArticles == 'asc'}" />
								<span>Articles</span>
								<ChevronDownIcon @click="setFilter('sortByArticles','desc')" class="h-4 w-4 ml-1 text-gray-500 hover:text-amber-700 cursor-pointer" :class="{'text-amber-700': form.sortByArticles == 'desc'}" />
							</div>
							</th>
					...
					<tbody class="pt-4">
						<tr v-for="category in reactiveCategories" :key="category.id" class="border-t space-x-2 text-center hover:bg-neutral-200 cursor-pointer" @click="visit(category.id)">
...
<script>
	...
	export default{
		...
		setup(props) {
			...
			const visit = (id) => {
				Inertia.get(route('categories.edit', id));
			}

			watch(form, () => {
				axios.post(route('categories.search'), form)
					.then(res => {
			...
			return { form, setFilter, visit, reactiveCategories }
		}
	}
</script>