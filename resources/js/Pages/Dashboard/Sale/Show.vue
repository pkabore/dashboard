<template>
	<div class="mx-auto max-w-md family-mono">
		<h2 class="text-xl font-bold mt-4 text-gray-600 text-center">Reçu N°{{sale.receipt_id}}</h2>
    <div class="pt-4 w-full my-7 px-4 ml-3 bg-white rounded-md h-full mx-auto">
      ...
      <div class="w-full mx-auto overflow-x-hidden mt-4 p-0">
          <table class="table-auto w-full p-0">
          	...
          	<tbody>
          		<tr v-for="(order, i) in sale.items" :key="i" class="break-words text-center">	
    	...
    	<div v-if="sale.items.length" class="mt-4 py-2 w-full border-t-2 border-dashed text-lg border-gray-300">
    		...
    		<p class="border-t-2 border-dashed mt-2 px-0 py-4 text-center">Au revoir et à bientôt...</p>
    	</div>
    </div>
  </div>
</template>
